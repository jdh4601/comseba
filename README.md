...
1. 학생 이름 입력
2. **학교급 선택** — 중등 (중학생) / 고등 (고등학생). 매 세션 시작 시 묻습니다
3. 신규 / 기존 세션 선택 (이전 세션은 완료된 단계를 건너뛰고 재개)
4. 학생 프로필 — 텍스트 / HWP·HWPX 파일 / 혼합 + (선택) 카카오톡 스크린샷
5. **과목 선택** — 국어 / 영어 / 수학 / 과학 / 역사 / 진로 / 독서 / 직접 입력
6. 평가기준 이미지 업로드 → 추출된 루브릭 교사 확인
7. 수행평가 아이디어 제안 (스킵 가능)
8. 제출물 입력 (PDF / 이미지 파일 또는 직접 텍스트)
9. 항목별 피드백 + 예시 답안 + Markdown 보고서 + 학부모 문자 초안 자동 저장

**과목**과 **학교급** 컨텍스트는 모든 LLM 호출의 system prompt 에 주입돼 평가 항목 / 어휘 /
모범 답안 분량 / 학부모 문자 어조가 대상 학생 수준과 과목 특성에 맞춰집니다.
